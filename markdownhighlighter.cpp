...
void MarkdownHighlighter::initTextFormats(int defaultFontSize) {
    ...
    // set character format for math blocks
    format = QTextCharFormat();
    format.setFont(QFontDatabase::systemFont(QFontDatabase::FixedFont));
    format.setBackground(QColor(220, 220, 220));
    format.setFontWeight(QFont::StyleItalic);
    format.setFontItalic(true);
    _formats[MathBlock] = format;
...
void MarkdownHighlighter::highlightMarkdown(const QString &text) {
    ...
    else if (isBlockMathBlock)
        highlightMathFence(text);
...
QPair<int, int> MarkdownHighlighter::findPositionInRanges(
    MarkdownHighlighter::RangeTypes type, int blockNum, int pos) const {
...
bool MarkdownHighlighter::isPosInRangeType(int blockNumber, int position,
                                           RangeTypes type) const {
    const QVector<InlineRange> rangeList = _ranges.value(blockNumber);
    return std::find_if(rangeList.cbegin(), rangeList.cend(),
                        [position, type](const InlineRange &range) {
...
void MarkdownHighlighter::highlightEmAndStrong(const QString &text,
    ...
    for (int i = pos; i < text.length(); ++i) {
        ...
        bool isInRange =
            isPosInRangeType(currentBlock().blockNumber(), i,
                             RangeType::CodeSpan | RangeType::Math);
...
void MarkdownHighlighter::highlightMathFence(const QString &text) {
    if (text == QLatin1String("$$")) {
        ...
        setCurrentBlockState(previousBlockState());
        setFormat(0, text.length(), _formats[MathBlock]);
    }
}

int MarkdownHighlighter::highlightInlineMath(const QString &text,
                                             int currentPos) {
    clearRangesForBlock(currentBlock().blockNumber(), RangeType::Math);

    if (currentPos != 0 && text.at(currentPos - 1) == QLatin1Char('\\'))
        return currentPos;
    int i = currentPos;
    int mathEnd = i + 1;
    int pos = mathEnd;
    while ((mathEnd = text.indexOf(QLatin1Char('$'), pos)) != -1 &&
           text.at(mathEnd - 1) == QLatin1Char('\\')) {
        pos = mathEnd + 1;
    }
    if (mathEnd == -1) return currentPos;

    int end = mathEnd - 1;
    if (i == end) return end;

    _ranges[currentBlock().blockNumber()].append(
        InlineRange(i, end, RangeType::Math));

    setFormat(i, 1, _formats[MaskedSyntax]);
    setFormat(mathEnd, 1, _formats[MaskedSyntax]);
    setFormat(i + 1, end - i, _formats[MathBlock]);
    return mathEnd;
}
